...
@Component({
  selector: 'funding-rates-coin-panel',
  templateUrl: './funding-rates-coin.component.html',
  styleUrls: ['./funding-rates-coin.component.scss']
})
export class FundingRatesCoinComponent implements OnInit, AfterViewInit {
...
//    "Kraken",
  ...
  async refresh() {
    ...
    for (let xIndex = 0; xIndex < x.length; xIndex++) {
      ...
      const data = await x[xIndex].getFuningRateCoin();
    ...
    this.xDelta.startCoin(this.onDeltaFundingRate);
    //this.xCrypto.startCoin(this.onCryptoFundingRate);
  }
